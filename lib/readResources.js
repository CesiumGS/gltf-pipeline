...
function readResources(gltf, options) {
    ...
    options.decodeImages = defaultValue(options.decodeImages, false) || options.checkTransparency;
...
function readImage(gltf, image, options) {
    return readResource(gltf, image, options)
        .then(function(data) {
            ...
            image.extras._pipeline.transparent = false;
            if (options.decodeImages) {
                return generateJimpImage(image, options);
            }
        });
}
...
function readFile(object, uri, options) {
    ...
    if (path.isAbsolute(uri)) {
        ...
        if (!defined(resourceDirectory)) {
            return Promise.reject(new RuntimeError('glTF model references external files but no resourceDirectory is supplied'));
        }
        relativePath = uri;
