 ...
 * @param {Number} imageId The id of the image to compress.
 * @param {Object} options Options defining compressed texture setting. See {@link compressTextures}.
 * @returns {Promise} A promise that resolves to a buffer containing the compressed texture.
 *
...
function compressTexture(gltf, imageId, options) {
    ...
    var extension = getImageExtension(source);
    ...
    if (defined(jimpImage)) {
        ...
        if ((format === 'pvrtc1' || format === 'pvrtc2') && (!isSquare || !isPowerOfTwo)) {
            console.log('Warning: image ' + imageId + ' will be resized to the lower square power-of-two for pvrtc texture compression. To prevent this warning in the future resize any images beforehand.');
        } else if (!isPowerOfTwo) {
            console.log('Warning: image ' + imageId + ' will be resized to the lower power-of-two for texture compression. To prevent this warning in the future resize any images beforehand.');
        }
    }
    ...
    return fsExtra.ensureDir(tempDirectory)
        .then(function() {
            if (compressRaw) {
...
function compressWithAstcenc(inputPath, tempDirectory, options) {
    ...
    return runCompressTool(astcencPath, cliOptions, outputPath)
        .then(function(buffer) {
            return astcToKtx(buffer, blockSize);
        });
}
...
function astcToKtx(astcBuffer) {
    ...
    var ktxHeader = Buffer.alloc(68);

    var identifier = [0xAB, 0x4B, 0x54, 0x58, 0x20, 0x31, 0x31, 0xBB, 0x0D, 0x0A, 0x1A, 0x0A];
    for (var i = 0; i < 12; ++i) {
