...
function processJSON(gltfWithExtras, callback) {
...
function processJSONToDisk(gltfWithExtras, outputPath, isBinary, isEmbedded) {
    processJSON(gltfWithExtras, function(gltfWithExtras) {
        if (isBinary) {
            writeBinaryGltf(gltfWithExtras, outputPath, true);
        } else {
            writeGltf(gltfWithExtras, outputPath, isEmbedded, true);
        }
    });
}

function processFileToDisk(inputPath, outputPath, isBinary, isEmbedded) {
    processFile(inputPath, function(gltf) {
        if (isBinary) {
