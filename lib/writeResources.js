...
function writeBufferView(gltf, object) {
    ...
    if (defined(object.bufferView)) {
        // TODO : this assumes no changes were made to object.extras._pipeline and the buffer view still occupies the same length
        return;
    }
    var source = object.extras._pipeline.source;
...
function getProgram(gltf, shaderIndex) {
    var programHandler = function(program, index) {
    ...
    ForEach.program(gltf, programHandler);
    ForEach.programLegacy(gltf, programHandler);
}
