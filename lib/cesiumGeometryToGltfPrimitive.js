...
function mapGeometryAttributeToPrimitive(gltf, primitive, geometry, semantic, packedLength) {
    var attributeSemantic;
    var values;

    switch(semantic) {
        case 'position':
            attributeSemantic = getFirstAttributeSemantic(gltf, primitive, 'POSITION', packedLength);
            values = geometry.attributes.position.values;
            break;
        case 'normal':
            attributeSemantic = getFirstAttributeSemantic(gltf, primitive, 'NORMAL', packedLength);
            values = geometry.attributes.normal.values;
            break;
        case 'st':
            attributeSemantic = getFirstAttributeSemantic(gltf, primitive, 'TEXCOORD', packedLength);
...
function cesiumGeometryToGltfPrimitive(gltf, primitive, geometry) {
    ...
    var packedAttributeLength = attributes.position.values.length;
    for (var semantic in attributes) {
        if (attributes.hasOwnProperty(semantic)) {
             mapGeometryAttributeToPrimitive(gltf, primitive, geometry, semantic, packedAttributeLength);
        }
    }
