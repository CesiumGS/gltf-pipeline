...
function getFirstAttributeSemantic(gltf, primitive, semantic, packedLength) {
    ...
    if (semantics.length <= 0) {
        primitive.attributes[semantic] = createAccessor(gltf, packedLength, type,
            WebGLConstants.FLOAT, WebGLConstants.ARRAY_BUFFER);
        return semantic;
    }
    return semantics[0];
}
...
function mapGeometryAttributeToPrimitive(gltf, primitive, geometry, semantic) {
    ...
    switch(semantic) {
        ...
        case 'tangent':
            ...
            break;
        //TODO: refactor this case soon as it got renamed in Cesium
        //      (should probably become 'bitangent' and geometry.attributes.bitangent.values)
        case 'binormal':
            attributeSemantic = getFirstAttributeSemantic(gltf, primitive, 'BITANGENT', packedAttributeLength);
            values = geometry.attributes.binormal.values;
            break;
        default:
