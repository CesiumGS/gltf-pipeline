...
function compressDracoMeshes(gltf, options) {
    ...
    var uncompressedFallback = defaultValue(options.dracoOptions.uncompressedFallback, defaults.uncompressedFallback);
...
function addCompressionExtensionToPrimitive(gltf, primitive, attributeToId, dracoEncodedBuffer, uncompressedFallback) {
    ...
    var buffer = {
        byteLength: dracoEncodedBuffer.buffer.length,
        extras: {
            _pipeline: {
                source: dracoEncodedBuffer.buffer
            }
        }
    };
