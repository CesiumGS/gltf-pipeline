...
function mergeBuffers(gltf, defaultName, force) {
    ...
    ForEach.buffer(gltf, function(buffer, bufferId) {
        ...
        const extensions = defaultValue(buffer.extensions, defaultValue.EMPTY_OBJECT);
    ...
    forEachBufferViewLikeObject(gltf, function(bufferView) {
        ...
        const sourceBufferViewData = Buffer.from(source.slice(bufferView.byteOffset, bufferView.byteOffset + bufferView.byteLength));
...
function forEachBufferViewLikeObject(gltf, callback) {
    ForEach.bufferView(gltf, function(bufferView) {
        ...
        const extensions = defaultValue(bufferView.extensions, defaultValue.EMPTY_OBJECT);
        const meshoptObject = extensions.EXT_meshopt_compression;
