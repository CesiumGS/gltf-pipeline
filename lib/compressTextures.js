    ...
    for (var imageId in images) {
        if (images.hasOwnProperty(imageId)) {
            ...
            if (defined(jimpImage)) {
                ...
                resize = resizeToPowerOfTwo && (!CesiumMath.isPowerOfTwo(width) || !CesiumMath.isPowerOfTwo(height));
            }

            var promise;
            if (resize || flipY || imageChanged || (inputExtensions.indexOf(extension) === -1)) {
