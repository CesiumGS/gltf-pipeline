...
function compressTextures(gltf, options) {
    ...
    for (var imageId in images) {
        if (images.hasOwnProperty(imageId)) {
            ...
            var promise;
            if (resizeToPowerOfTwo || imageChanged || (inputExtensions.indexOf(extension) === -1)) {
...
function fixKTX(compressed) {
    // Crunch sets glTypeSize to 0 but it should be 1
    if (compressed.extension === '.ktx') {
        compressed.buffer.writeUInt32LE(1, 20);
    }
}
