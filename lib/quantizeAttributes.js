...
function quantizeAttributes(gltf, options) {
    ...
    // Generate quantized attributes and table for repacking the buffer
...
function createDecodeMatrix3(min, max, range) {
    return [(max[0] - min[0])/range, 0,                       0,
            0,                       (max[1] - min[1])/range, 0,
            min[0],                   min[1],                 1
    ];
}

function createDecodeMatrix4(min, max, range) {
    return [(max[0] - min[0])/range, 0,                       0,                       0,
            0,                       (max[1] - min[1])/range, 0,                       0,
            0,                       0,                       (max[2] - min[2])/range, 0,
            min[0],                  min[1],                  min[2],                  1
    ];
}
...
function createDecodeMatrix5(min, max, range) {
    return [(max[0] - min[0])/range, 0,                       0,                       0,                       0,
            0,                       (max[1] - min[1])/range, 0,                       0,                       0,
            0,                       0,                       (max[2] - min[2])/range, 0,                       0,
            0,                       0,                       0,                       (max[3] - min[3])/range, 0,
                                ...
                                if (attributeIsQuantizable(attribute)) {
                                    ...
                                    if (defined(visitedAccessors[accessorId])) {
                                        ...
                                        if (accessorIsQuantizable(accessor)) {
                                            accessorAttributes[accessor] = attribute;
                                        }
