...
function quantizeAttributes(gltf, options) {
                            ...
                            if (defined(attributes)) {
                                for (attribute in attributes) {
                                    if (attributes.hasOwnProperty(attribute)) {
                                        if (attribute === "NORMAL" || attribute === "POSITION") {
                                            accessorIds.push(attributes[attribute]);
                                        }
                                    }
                                }
                            }
    ...
    for (i = 0; i < accessorIds.length; i++) {
        var accessorId = accessorIds[i];
        ...
        for (j = 0; j < data.length; j++) {
            index = j % min.length;
            encoded[j] = (data[j] - min[index])*precision;
        }
        ...
        var byteSizeDifference = length - encoded.length * 2;
        ...
        output.set(source.slice(offset + length), offset + encoded.length * 2);

        accessor.byteStride = 6;
