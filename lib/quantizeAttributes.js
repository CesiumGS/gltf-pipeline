...
function quantizeAttributes(gltf, options) {
    ...
    for (var accessorId in accessorIds) {
        if (accessorIds.hasOwnProperty(accessorId)) {
            ...
            var data = accessorReader.read();
            while (defined(data)) {
                for (i = 0; i < numberOfComponents; i++) {
                    data[i] = Math.ceil((data[i] - min[i]) * range / (max[i] - min[i]));
                }
                accessorReader.write(data, newComponentType);
                accessorReader.next();
                data = accessorReader.read();
            }
                    ...
                    if (defined(attributes)) {
                        for (var attribute in attributes) {
                            if (attributes.hasOwnProperty(attribute)) {
                                if (!defined(validSemantics) || defined(validSemantics[attribute])) {
