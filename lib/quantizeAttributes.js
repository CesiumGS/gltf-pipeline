...
function quantizeAttributes(gltf, options) {
    ...
    for (var accessorId in accessorIds) {
        if (accessorIds.hasOwnProperty(accessorId)) {
            attribute = accessorIds[accessorId];
            ...
            /*var precision = 1e12;
            for (j = 0; j < decodeMatrix.length; j++) {
                var value = decodeMatrix[j];
                decodeMatrix[j] = Math.floor(value * precision)/precision;
            }
            for (j = 0; j < min.length; j++) {
                var value = min[j];
                min[j] = Math.floor(value * precision)/precision;
            }
            for (j = 0; j < max.length; j++) {
                var value = max[j];
                max[j] = Math.floor(value * precision)/precision;
            }*/
