...
function parseArguments(args) {
    var argv = yargs
        ...
        .options({
            ...
            'texcomp.format': {
                ...
                describe: 'The compressed texture format. Multiple formats may be supplied by repeating this flag.',
    ...
    // If multiple compressed texture formats are specified send in an array of options
    if (defined(textureCompressionOptions) && isArray(textureCompressionOptions.format)) {
        var formats = textureCompressionOptions.format;
        var length = formats.length;
        var optionsArray = new Array(length);
        for (i = 0; i < length; ++i) {
            var options = clone(textureCompressionOptions);
            options.format = formats[i];
            optionsArray[i] = options;
        }
        textureCompressionOptions = optionsArray;
    }
