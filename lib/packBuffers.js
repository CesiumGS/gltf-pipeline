...
function packBuffers(gltf) {
    ...
    for (var bufferId in buffers) {
        if (buffers.hasOwnProperty(bufferId)) {
            packBuffer(gltf, bufferId, packBufferViews);
        }
    }
...
function packBuffer(gltf, bufferId, packBufferViews) {
    var buffers = gltf.buffers;
    var buffer = buffers[bufferId];
    var source = buffer.extras._pipeline.source;
    var packBuffer = new Uint8Array(source.length);
    var accessors = accessorsByTargetAndByteLength(gltf, bufferId);
    var offset = 0;
...
function accessorsByTargetAndByteLength(gltf, bufferId) {
    var accessors = gltf.accessors;
    var bufferViews = gltf.bufferViews;
