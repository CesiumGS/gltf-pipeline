 ...
 * @param {Array.<Number>} data The data to store in the accessor buffer.
 ...
 */
function createAccessor(gltf, data, type, componentType, target, id) {
    var numberOfComponents = numberOfComponentsForType(type);
    if (data.length % numberOfComponents !== 0) {
    ...
    var bufferData = new Buffer(data.length * componentByteLength);
    for (var i = 0; i < data.length; i++) {
        writeBufferComponent(bufferData, componentType, data[i], componentByteLength);
    }
    var accessor = {
    ...
    var minMax = findAccessorMinMax(gltf, id);
