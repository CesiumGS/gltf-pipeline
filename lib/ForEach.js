...
ForEach.arrayOfObjects = function(arrayOfObjects, handler) {
    for (var i = 0; i < arrayOfObjects.length; i++) {
        ...
        var offset = defaultValue(handler(i, object), 0);
        i += offset;
    }
};
    ...
    if (defined(attributes)) {
        for (var semantic in attributes) {
            if (attributes.hasOwnProperty(semantic)) {
                handler(semantic, attributes[semantic]);
            }
        }
    }
};
...
ForEach.nodeInScene = function(gltf, sceneId, handler) {
    ...
    if (defined(parameters)) {
        for (var parameterName in parameters) {
            if (parameters.hasOwnProperty(parameterName)) {
                handler(parameterName, parameters[parameterName]);
            }
        }
    }
};
