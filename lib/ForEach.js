...
ForEach.materialValueLegacy = function (material, handler) {
...
ForEach.programLegacy = function (gltf, handler) {
    if (defined(gltf.programs) && !isArray(gltf.programs)) {
        ForEach.objectLegacy(gltf.programs, handler);
    } else {
...
ForEach.shaderLegacy = function(gltf, handler) {
    if (defined(gltf.shaders) && !isArray(gltf.shaders)) {
...
ForEach.techniqueAttributeLegacy = function(technique, handler) {
    var attributes = technique.attributes;
    for (var attributeName in attributes) {
        if (attributes.hasOwnProperty(attributeName)) {
            // Gets parameter name
...
ForEach.techniqueUniformLegacy = function (technique, handler) {
    var uniforms = technique.uniforms;
    for (var uniformName in uniforms) {
        if (uniforms.hasOwnProperty(uniformName)) {
            // Gets paramter name
...
ForEach.techniqueLegacy = function (gltf, handler) {
    if (defined(gltf.techniques) && !isArray(gltf.techniques)) {
        ForEach.objectLegacy(gltf.techniques, handler);
    } else {
