...
const getImageWebp = require('./getImageWebp');
...
function decrementBufferView(object, bufferViewId) {
    if (defined(object.bufferView) && object.bufferView > bufferViewId) {
        object.bufferView--;
    }
}
...
Remove.bufferView = function(gltf, bufferViewId) {
    ...
    ForEach.accessor(gltf, function(accessor) {
        decrementBufferView(accessor, bufferViewId);
    });

    ForEach.shader(gltf, function(shader) {
        decrementBufferView(shader, bufferViewId);
    });

    ForEach.image(gltf, function(image) {
        const imageWebp = getImageWebp(image);
        if (defined(imageWebp)) {
            decrementBufferView(imageWebp, bufferViewId);
        }

        decrementBufferView(image, bufferViewId);

        ForEach.compressedImage(image, function(compressedImage) {
            const compressedImageBufferView = compressedImage.bufferView;
            decrementBufferView(compressedImageBufferView, bufferViewId);
        });
    });
...
getListOfElementsIdsInUse.bufferView = function(gltf) {
    ...
    ForEach.image(gltf, function(image) {
        const imageWebp = getImageWebp(image);
        if (defined(imageWebp)) {
            usedBufferViewIds[imageWebp.bufferView] = true;
        }
