...
function parseBinaryGltf(data) {
    ...
    loadSourceFromBody(gltf, gltf.images, 'images', body, usedBufferViews);
    loadSourceFromBody(gltf, gltf.shaders, 'shaders', body, usedBufferViews);
    ...
    for (var imageId in images) {
        if (images.hasOwnProperty(imageId)) {
            ...
            if (defined(image.extras) && defined(image.extras.compressedImage3DTiles)) {
                ...
                loadSourceFromBody(gltf, compressedImages, 'images', body, usedBufferViews);
            }
        }
    }
...
function loadSourceFromBody(gltf, objects, name, body, usedBufferViews) {
    if (defined(objects)) {
