...
function combinePrimitivesInGroup(gltf, allPrimitives, primitives, materialId, mode, uint32Enabled) {
    ...
    if (primitivesLength > 1) {
        ...
        // glTF 1.x: subdivide groups for concatenatePrimitives if the combined primitive will overflow uint16 (65535)
...
function SortablePrimitive(primitive, accessors) {
    ...
    this.indicesCount = accessors[primitive.indices].count;
    this.vertexCount = getVertexCount(primitive, accessors);
}

function compareIndicesCounts(primitiveA, primitiveB) {
    return primitiveB.indicesCount - primitiveA.indicesCount;
}
...
function subdividePrimitiveGroup(primitiveGroup, accessors, indexMax) {
    ...
    // assuming that the largest index in each primitive is its vertex count.
    var indexOffset = 0;
    ...
    // "bin" primitives together, creating a new "bin" when the estimated new index will overflow uint16.
    var sortablePrimitives = new Array(primitiveCount);
    for (i = 0; i < primitiveCount; i++) {
        sortablePrimitives[i] = new SortablePrimitive(primitiveGroup[i], accessors);
    }
    sortablePrimitives.sort(compareIndicesCounts);

    var currentBucket = [];
    var buckets = [currentBucket];
    indexOffset = 0;
    while(sortablePrimitives.length > 0) {
        var sortablePrimitive = sortablePrimitives.pop();
        ...
        if (vertexCount - 1 + indexOffset > indexMax) {
            ...
            // put it in a new bucket and restart the overflow counter.
