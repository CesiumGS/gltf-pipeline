...
function loadGltfUris(gltf, options) {
    ...
    var loadPromises = [
        loadURIs(gltf.buffers, 'buffers', basePath),
    ...
    /* jshint unused:vars */
    ForEach.image(function(imageId, image) {
        if (defined(image.extras) && defined(image.extras.compressedImage3DTiles)) {
            ...
            loadPromises.push(loadURIs(compressedImages, 'images', basePath));
        }
    });
...
function loadURIs(objects, name, basePath) {
    if (defined(objects)) {
        return Promise.each(objects, function (object) {
            return loadURI(object, name, basePath);
        });
    }
}

function loadURI(object, name, basePath) {
    var promises = [];
