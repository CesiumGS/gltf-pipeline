...
function loadGltfUris(gltf, options) {
    ...
    // glTF 1.0 compatibility
    if (defined(buffers) && !Array.isArray(buffers)) {
        buffers = Object.keys(buffers).map(function(key) {
            return buffers[key];
        });
    }
    var images = gltf.images;
    if (defined(images) && !Array.isArray(images)){
        images = Object.keys(images).map(function(key) {
            return images[key];
        });
    }
    var shaders = gltf.shaders;
    if (defined(shaders) && !Array.isArray(shaders)) {
        shaders = Object.keys(shaders).map(function(key) {
            return shaders[key];
        });
    }
    return loadURIs(gltf, buffers, 'buffers', basePath)
        .then(function() {
            var loadPromises = [
                loadURIs(gltf, images, 'images', basePath),
                loadURIs(gltf, shaders, 'shaders', basePath)
            ];
