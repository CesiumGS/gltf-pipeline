...
function objectsToArrays(gltf) {
    ...
    ForEach.shader(gltf, function(shader) {
        ...
        if (defined(extensions)) {
            ...
            if (defined(binaryGltf)) {
                shader.bufferView = binaryGltf.bufferView;
    ...
    ForEach.animation(gltf, function(animation) {
        ...
        if (defined(channels)) {
            ...
            for (i = 0; i < channelsLength; i++) {
                ...
                if (defined(target)) {
                    target.node = globalMapping.nodes[target.node];
                }
            }
        }
    });
    ForEach.material(gltf, function(material) {
        ForEach.materialValue(material, function(value, name) {
            if (Array.isArray(value)) {
                ...
                material.value[name] = globalMapping.textures[value];
            }
        });
    });
    ForEach.image(gltf, function(image) {
        ...
        if (defined(extensions)) {
            var binaryGltf = extensions.KHR_binary_glTF;
            if (defined(binaryGltf)) {
                image.bufferView = binaryGltf.bufferView;
