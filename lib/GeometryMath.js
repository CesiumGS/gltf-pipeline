...
function sumBarycentric(barycentric, vector0, vector1, vector2, result) {
    ...
    if (!defined(barycentric)) {
        throw new DeveloperError('in GeometryMath: barycentric is required');
    }
    if (!defined(vector0)) {
        throw new DeveloperError('in GeometryMath: vector0 is required');
    }
    if (!defined(vector1)) {
        throw new DeveloperError('in GeometryMath: vector1 is required');
    }
    if (!defined(vector2)) {
        throw new DeveloperError('in GeometryMath: vector2 is required');
    }
    if (!defined(result)) {
        throw new DeveloperError('in GeometryMath: result is required');
    }
    //>>includeEnd('debug');
...
function pointLineDistanceParametric(point, linePosition, lineDirection) {
    //>>includeStart('debug', pragmas.debug);
    if (!defined(point)) {
        throw new DeveloperError('in GeometryMath: point is required');
    }
    if (!defined(linePosition)) {
        throw new DeveloperError('in GeometryMath: linePosition is required');
    }
    if (!defined(lineDirection)) {
        throw new DeveloperError('in GeometryMath: lineDirection is required');
    }
    //>>includeEnd('debug');
...
function flattenTriangle(positions, results) {
    ...
    if (!defined(positions)) {
        throw new DeveloperError('in GeometryMath: positions is required');
    }
    if (!defined(results)) {
        throw new DeveloperError('in GeometryMath: results is required');
    }
    //>>includeEnd('debug');
