...
function premultiplyTransformWithBuffer(matrix, buffer, attributeId, min, max) {
    ...
    while (bufferIndex < bufferLength) {
        ...
        var newVector = new Cartesian3();

        if (attributeId === 'POSITION') {
            Matrix4.multiplyByPoint(matrix, currentVector, newVector);
        } else if (attributeId === 'NORMAL') {
            var inverse = Matrix4.inverse(matrix, new Matrix4());
            var inverseTranspose = Matrix4.transpose(inverse, new Matrix4());
            Matrix4.multiplyByPointAsVector(inverseTranspose, currentVector, newVector);
        } else {
            Matrix4.multiplyByPointAsVector(matrix, currentVector, newVector);
        }

        resultBuffer.writeFloatLE(newVector.x, bufferIndex);
        resultBuffer.writeFloatLE(newVector.y, bufferIndex + 4);
        resultBuffer.writeFloatLE(newVector.z, bufferIndex + 8);

        //Update the min and max values if defined.
        if (defined(min)) {
            min = Cartesian3.minimumByComponent(min, newVector, min);
        }
        if (defined(max)) {
            max = Cartesian3.maximumByComponent(max, newVector, max);
        }
