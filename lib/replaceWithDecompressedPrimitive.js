    ...
    for (const semantic in dracoAttributes) {
        if (Object.prototype.hasOwnProperty.call(dracoAttributes, semantic)) {
            ...
            if (uncompressedFallback) {
                ...
                const bufferObject = getAttributeBuffer(decoderModule, decoder, dracoGeometry, attributeId);
                attributeAccessor.min = bufferObject.minValues;
                attributeAccessor.max = bufferObject.maxValues;

                attributeAccessor.bufferView = addBuffer(gltf, bufferObject.attributeBuffer);
                attributeAccessor.byteOffset = 0;
            } else if (defined(attributeAccessor.min) && defined(attributeAccessor.max)) {
                const minmax = calculateQuantizedMinMax(semantic, attributeAccessor, quantizationBitsValues);
...
function calculateQuantizedMinMax(semantic, attributeAccessor, quantizationBitsValues) {
    ...
    let quantizedMin = Array(numberOfComponents).fill(0);
    let quantizedMax = Array(numberOfComponents).fill(0);
    for (let c = 0; c < numberOfComponents; ++c) {
    ...
    let dequantizedMin = Array(numberOfComponents).fill(0);
    let dequantizedMax = Array(numberOfComponents).fill(0);
    for (let c = 0; c < numberOfComponents; ++c) {
