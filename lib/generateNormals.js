...
function generateFaceNormals(gltf, primitive) {
    ...
    var batchIdSemantic = "BATCHID";
    var batchIdAccessorId = attributes[batchIdSemantic];
    var batchIdAccessor = accessors[batchIdAccessorId];
    var batchIds = [];
    readAccessor(gltf, batchIdAccessor, batchIds);
    var indices;
    var indicesAccessorId = primitive.indices;
    var indicesAccessor;
    ...
    for (var i = 0; i < indicesLength; i+=3) {
        ...
        generatedNormals.push(normal);
        generatedNormals.push(normal);
        generatedBatchIds.push(batchIds[indices[i]]);
        generatedBatchIds.push(batchIds[indices[i+1]]);
        generatedBatchIds.push(batchIds[indices[i+2]]);
    }
    ...
    attributes.BATCHID = generateScalarUnsignedShortAccessor(gltf, generatedBatchIds);
    if (defined(indicesAccessorId)) {
