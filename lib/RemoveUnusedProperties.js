...
function removeNode(gltf, nodeId) {
    ...
    for (var i = 0; i < nodesLength; i++) {
        ...
        if (defined(skeletons)) {
            ...
            for (j = 0; j < skeletonsLength; j++) {
                var skeletonId = skeletons[j];
                if (skeletonId > nodeId) {
                    skeletons[j] = skeletonId - 1;
                }
            }
        }
    }
        ...
        for (var techniqueId = 0; techniqueId < techniquesLength; techniqueId++) {
            ...
            for (var parameterName in parameters) {
                if (parameters.hasOwnProperty(parameterName)) {
                    ...
                    if (defined(parameter.node) && parameter.node > nodeId) {
                        parameter.node--;
                    }
                }
            }
    ...
    for (var sceneId = 0; sceneId < scenesLength; sceneId++) {
        ...
        if (defined(sceneNodes)) {
            ...
            for (j = 0; j < sceneNodesLength; j++) {
                var sceneNodeId = sceneNodes[j];
                if (sceneNodeId > nodeId) {
                    sceneNodes[j] = sceneNodeId - 1;
                }
            }
        }
    }
...
function removeMesh(gltf, meshId) {
    var nodes = gltf.nodes;

    gltf.meshes.splice(meshId, 1);
    var nodesLength = nodes.length;
    for (var nodeId = 0; nodeId < nodesLength; nodeId++) {
        var node = nodes[nodeId];
        var nodeMeshId = node.mesh;
        if (defined(nodeMeshId) && nodeMeshId > meshId) {
            node.mesh--;
        }
    }
}
    ...
    if (defined(nodes)) {
        for (var nodeId in nodes) {
            if (nodes.hasOwnProperty(nodeId)) {
                var node = nodes[nodeId];
                var nodeMeshId = node.mesh;
                if (defined(nodeMeshId)) {
                    var mesh = meshes[nodeMeshId];
                    if (!defined(mesh.primitives) || mesh.primitives.length === 0) {
                        delete node.mesh;
                    } else {
                        usedMeshIds[nodeMeshId] = true;
                    }
                }
            }
        }
