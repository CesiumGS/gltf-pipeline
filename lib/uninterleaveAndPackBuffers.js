...
function packAccessorsForTarget(gltf, bufferId, target, accessorsByByteLength, sourceBuffer, packBuffer, packBufferViews, offset) {
    ...
    for (var i = 0; i < byteLengths.length; i++) {
        ...
        if (defined(accessorIds) && accessorIds.length > 0) {
            ...
            packOffset = packAccessors(gltf, accessorIds, sourceBuffer, packBuffer, bufferViewId, packOffset);
        }
    }
    var packBufferView = {
        buffer : bufferId,
...
function packAccessors(gltf, accessorsToPack, sourceBuffer, packBuffer, bufferViewId, packOffset) {
