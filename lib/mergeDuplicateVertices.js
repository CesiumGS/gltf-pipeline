    ...
    for (var indexAccessorId in indexAccessors) {
        if (indexAccessors.hasOwnProperty(indexAccessorId)) {
            ...
            for (var i = 0; i < numIndices; i++) {
                var index = readBufferComponentType(source, accessor.componentType, byteOffset + byteStride * i);
                var mappedIndex = duplicateMapping[index];
                if (defined(mappedIndex) && mappedIndex !== index) {
                    writeBufferComponentType(source, accessor.componentType, mappedIndex, byteOffset + byteStride * i);
                }
            }
        }
    }
    ...
    for (var n = 0; n < primitivesLength; n++) {
        ...
        if (defined(indexAccessorId)) {
            ...
            for (var i = 0; i < indexAccessor.count; i++) {
                var index = readBufferComponentType(indexSource, indexComponentType, indexByteOffset + indexByteStride * i);
                ...
                for (var k = 0; k < numAttributes; k++) {
                    ...
                    if (k === numAttributes - 1) {
                        if (!defined(subTree[value])) {
                            ...
                            if (!defined(mapping) || (defined(mapping) && originalIndex === mapping)) {
                                duplicateMapping[index] = originalIndex;
                            } else {
                                // This value was already mapped to a different index for a different set of attributes
                                duplicateMapping[index] = index;
                                subTree[value] = index;
                            }
                        }
