...
function bakeAmbientOcclusion(gltf, options) {
    ...
    if (options.toVertex) {
        NodeHelpers.forEachPrimitiveInScene(gltf, scene, raytracePrimitiveAtTriangleCenters, parameters);
...
function raytracePrimitiveAtTriangleCenters(primitive, meshPrimitiveID, parameters, node) {
    var raytracerScene = parameters.raytracerScene;
    var aoBuffer = raytracerScene.aoBufferByPrimitive[meshPrimitiveID];
    ...
    for (var i = 0; i < numTriangles; i++) {
        ...
        // Raytrace
        var oldOcclusionAmt = aoBuffer.samples[index0];
        computeAmbientOcclusionAt(position, normal, numberSamples, sqrtNumberSamples,
            triangleSoup, raytracerScene.nearCull,
            raytracerScene.rayDepth, aoBuffer, index0);
        var diff = aoBuffer.samples[index0] - oldOcclusionAmt; // TODO: this is jank fix it

        aoBuffer.samples[index1] += diff;
        aoBuffer.samples[index2] += diff;
