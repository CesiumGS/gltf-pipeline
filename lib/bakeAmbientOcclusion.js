    ...
    for (var i = 0; i < techniquesCount; i++) {
        ...
        if (shaderMode !== 'multiply') {
            ...
            if (diffuseParameter.type === WebGLConstants.SAMPLER_2D) {
                // Find the technique attribute for the texture coordinate parameter, semantic TEXCOORD.
                var parameters = technique.parameters;
                var texcoordParameterId = '';
                for (var parameterId in parameters) {
                    if (parameters.hasOwnProperty(parameterId)) {
                        ...
                        if (defined(parameter.semantic)) {
                            if (parameter.semantic.indexOf('TEXCOORD') === 0) {
                                texcoordParameterId = parameterId;
                                break;
                            }
                        }
                    }
                ...
                for (var attributeName in attributes) {
                    if (attributes.hasOwnProperty(attributeName)) {
                        if (attributes[attributeName] === texcoordParameterId) {
