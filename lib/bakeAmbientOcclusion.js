...
function bakeAmbientOcclusion(gltf, options) {
    ...
    if (options.toVertex) {
        NodeHelpers.forEachPrimitiveInScene(gltf, scene, raytracePrimitiveAtVertices, parameters);
        // Add to the gltf
        bakeToVertices(gltf, scene, raytracerScene);
    } else {
...
function bakeToVertices(gltf, scene, raytracerScene) {
    ...
    var parameters = {
        ...
        // If an item is cloned, store the clone's name as the value.
        materialCloneIDs: {},
        techniqueCloneIDs: {},
        programCloneIDs: {},
    ...
    // Edit the shaders
    var vertexShadersToEdit = [];
    for (var shaderID in parameters.vertexShaderCloneIDs) {
        if (parameters.vertexShaderCloneIDs.hasOwnProperty(shaderID)) {
            vertexShadersToEdit.push(gltf.shaders[shaderID]);
        }
    }
    var fragmentShadersToEdit = [];
    for (shaderID in parameters.fragmentShaderCloneIDs) {
        if (parameters.fragmentShaderCloneIDs.hasOwnProperty(shaderID)) {
            fragmentShadersToEdit.push(gltf.shaders[shaderID]);
        }
    }
    editShaders(vertexShadersToEdit, WebGLConstants.VERTEX_SHADER);
    editShaders(fragmentShadersToEdit, WebGLConstants.FRAGMENT_SHADER);
    ...
    for (var programID in parameters.programCloneIDs) {
        if (parameters.programCloneIDs.hasOwnProperty(programID)) {
            ...
            program.attributes.push('a_ambientOcclusion');
        }
    }
...
function addVertexData(gltf, scene, raytracerScene) {
    ...
    for (var i = 0; i < primitiveCount; i++) {
        ...
        byteOffset += primitiveVertexCount;
    }
}

function editShaders(shadersToEdit, type) {
    for (var shaderID in shadersToEdit) {
        if (shadersToEdit.hasOwnProperty(shaderID)) {
            // Unpack to string
            var shader = shadersToEdit[shaderID];
            var sourceString = shader.extras._pipeline.source.toString();

            // Wrap main
            sourceString = ShaderSource.replaceMain(sourceString, 'main_before_AO');

            // Vertex: add an ao attribute and ao varying. pass it through.
            if (type === WebGLConstants.VERTEX_SHADER) {
                sourceString =
                    'attribute float a_ambientOcclusion; \n' +
            ...
            // Fragment: add an ao varying. use it to modulate the fragment color.
            if (type === WebGLConstants.FRAGMENT_SHADER) {
                sourceString =
                    'varying float v_ambientOcclusion; \n' +
                    sourceString + '\n' +
                    'void main() \n' +
                    '{ \n' +
                    '    main_before_AO(); \n' +
                    '    gl_FragColor.r *= (1.0 - v_ambientOcclusion); \n' +
                    '    gl_FragColor.g *= (1.0 - v_ambientOcclusion); \n' +
                    '    gl_FragColor.b *= (1.0 - v_ambientOcclusion); \n' +
                    '}';
            }

            // Repack into source
            shader.extras._pipeline.source = new Buffer(sourceString);
        }
    }
}
...
function processPrimitive(primitive, meshPrimitiveID, parameters, node) {
    ...
    if (raytracerScene.toVertex) {
        ...
        raytracerScene.aoBufferByPrimitive[meshPrimitiveID] = {
            ...
            count: new Array(vertexCount).fill(0)
        };
    } else {
...
function raytracePrimitiveTexels(primitive, meshPrimitiveID, parameters, node) {
    ...
    for (var i = 0; i < numTriangles; i++) {
        ...
        var barycentric = cartesian3Scratch;
        ...
        while(uStep < uMax) {
            ...
            while(vStep < vMax) {
                ...
                var texelPosition = worldPositionScratch;
                var texelNormal = worldNormalScratch;
                sumBarycentric(barycentric, positions[index0], positions[index1], positions[index2], texelPosition);
                sumBarycentric(barycentric, normals[index0], normals[index1], normals[index2], texelNormal);
...
// own but is also a preprocess for triangle averaged AO.
...
function raytracePrimitiveAtVertices(primitive, meshPrimitiveID, parameters, node) {
    var raytracerScene = parameters.raytracerScene;
    var aoBuffer = raytracerScene.aoBufferByPrimitive[meshPrimitiveID];
    ...
    for (var i = 0; i < vertexCount; i++) {
        ...
        // Raytrace
        computeAmbientOcclusionAt(position, normal, numberSamples, sqrtNumberSamples,
            triangleSoup, raytracerScene.nearCull,
            raytracerScene.rayDepth, aoBuffer, i);
    }
}
...
function generateJitteredRay(position, normal, sampleNumber, sqrtNumberSamples) {
    ...
    var axis = Cartesian3.cross(randomDirection, normal, cartesian3Scratch);
