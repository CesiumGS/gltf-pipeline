...
function cloneJsonAsNeeded(id, cloneIds, items) {
    ...
    if (defined(cloneID)) {
        if (cloneID === '') { // No clone exists yet. Make one!
            cloneID = addToArray(items, clone(items[id]));
            cloneIds[id] = cloneID;
        }
        return cloneID;
    }
...
function addVertexData(gltf, options) {
    ...
    var aoBufferId = addToArray(gltf.buffers, aoBuffer);
    ...
    var aoBufferViewId = addToArray(gltf.bufferViews, aoBufferView);
    ...
    for (var i = 0; i < primitiveCount; i++) {
        ...
        primitive.attributes._OCCLUSION = addToArray(gltf.accessors, aoAccessor);
...
function bakeToTexture(gltf, options) {
    ...
    for (var materialId = 0; materialId < materialsLength; materialId++) {
        ...
        if (defined(material.values) && defined(material.values.diffuse)) {
            if (isTexture(material.values.diffuse)) {
                exampleMaterialID = materialId;
                exampleTextureID = material.values.diffuse.index;
...
function cloneAndSetupMaterialTextureImage(options) {
    ...
    if (defined(options.oldImageID)) {
        ...
        options.newImageID = addToArray(images, newImage);
    }
    if (defined(options.oldTextureID)) {
        var oldTexture = textures[options.oldTextureID];
        newTexture = clone(oldTexture);
        newTexture.source = options.newImageID;
        options.newTextureID = addToArray(textures, newTexture);
        if (defined(options.oldMaterialID)) {
            newMaterial = clone(materials[options.oldMaterialID]);
            newMaterial.texture = options.newTextureID;
            options.newMaterialID = addToArray(materials, newMaterial);
        }
    }
}
...
function ensureImageUniqueness(gltf, options) {
    ...
    if (!defined(diffuse) || Array.isArray(diffuse)) {
        parameters = {
            newMaterialID : undefined,
            newTextureID : undefined,
        ...
        cloneAndSetupMaterialTextureImage(parameters);
        ...
        var newJimpImage = allImages[parameters.newImageID].extras._pipeline.jimpImage;
        ...
        primitive.material = parameters.newMaterialID;
        return allImages[parameters.newImageID];
    }

    var textureID = diffuse.index;
    ...
    if (materialsSeen.hasOwnProperty(materialID)) {
        ...
        cloneAndSetupMaterialTextureImage(parameters);
        texture.source = parameters.newImageID;
    } else {
