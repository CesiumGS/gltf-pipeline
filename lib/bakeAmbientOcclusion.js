...
function generateRaytracerScene(gltf, scene, options) {
    ...
    raytracerScene.triangleUniformGrid = buildTriangleStaticUniformGrid(allTriangles, raytracerScene.rayDepth);
...
function computeAmbientOcclusionAt(position, normal, numberSamples, sqrtNumberSamples,
    ...
    for (var j = 0; j < numberSamples; j++) {
        ...
        var nearestIntersect = uniformGridRaytrace(triangles, sampleRay, nearCull);
...
function raytraceNeighborFunction(positions, parameters) {
    ...
    if (defined(distance) && distance > parameters.nearCull) {
        parameters.minIntersect = Math.min(distance, parameters.minIntersect);
    }
}
