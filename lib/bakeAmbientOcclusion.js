...
var trianglePixelMarchScratch = {
    uv0 : cartesian2Scratch,
    uv1 : cartesian2Scratch,
    uv2 : cartesian2Scratch,
    position0 : worldPositionScratch,
    position1 : worldPositionScratch,
    position2 : worldPositionScratch,
    normal0 : worldNormalScratch,
    normal1 : worldNormalScratch,
    normal2 : worldNormalScratch,
    transform : matrix4Scratch,
    inverseTranspose : matrix4Scratch,
    raytracerScene : {},
    pixelWidth : 0.0,
    resolution : 0,
    callback : {},
...
var computeAmbientOcclusionAtScratch = {
    position : worldPositionScratch,
    normal : worldNormalScratch,
    numberRays : 0,
    sqrtNumberRays : 0,
    triangles : {},
    nearCull : 0.0,
...
function generateOptions(aoOptions) {
    if (!defined(aoOptions.gltfWithExtras)) {
        throw new DeveloperError('In stage bakeAmbientOcclusion: gltfWithExtras is required');
    }
    ...
    for (var i = 0; i < techniquesCount; i++) {
        ...
        if (shaderMode !== 'multiply') {
            ...
            if (!defined(diffuseParameter)) {
                throw new DeveloperError('In stage bakeAmbientOcclusion: Could not find parameter diffuse in technique ' + techniqueIds[i]);
            }
            ...
            if (diffuseParameter.type === WebGLConstants.SAMPLER_2D) {
                ...
                if (!defined(passUVThrough)) {
                    throw new DeveloperError('In stage bakeAmbientOcclusion: Could not find attribute a_texcoord0 in technique ' + techniqueIds[i]);
                }
