...
function bakeAmbientOcclusion(gltf, options) {
    ...
    if (options.toVertex) {
        ...
        //NodeHelpers.forEachPrimitiveInScene(gltf, scene, raytraceAtVertices, parameters);
        // Add to the gltf
...
function addVertexData(gltf, scene, raytracerScene) {
    ...
    // add a new buffer TODO: add option to append to a buffer?
        ...
        switch(options.shaderMode) {
            ...
            default: // mix
                glslChangeColor = 'gl_FragColor.rgb = mix(gl_FragColor.rgb, (1.0 - v_ambientOcclusion) * ' +
                    glslDiffuseColor + ', ' + defaultValue(options.blendAmount, 0.5) + '); \n';
                break;
        }
...
function generateRaytracerScene(gltf, scene, options) {
    ...
    var raytracerScene = {
        ...
        triangleAverage: options.triangleAverage,
    ...
    if (options.groundPlane) {
        ...
        // - should be a little lower than scene min, so rays cast from points at min can still hit it.
...
function pixelMarch(uv0, uv1, uv2, position0, position1, position2, normal0, normal1, normal2,
    ...
    while(uStep < uMax) {
        ...
        while(vStep < vMax) {
            ...
            if (defined(parameters.pixelIndex)) {
                parameters.pixelIndex = Math.floor(uStep / pixelWidth) + Math.floor(vStep / pixelWidth) * resolution;
            }

            callback(parameters, contribution, numberSamples, barycentric);
            vStep += pixelWidth;
        }
        uStep += pixelWidth;
    }
...
function texelCallback(parameters, contribution, numberSamples) {
    var texelIndex = parameters.pixelIndex;
    parameters.aoBuffer.count[texelIndex] += numberSamples;
    parameters.aoBuffer.samples[texelIndex] += contribution;
}
...
function raytraceToTexels(primitive, meshPrimitiveID, parameters, node) {
    ...
    for (var i = 0; i < numTriangles; i++) {
        ...
        pixelMarch(uvs[i0], uvs[i1], uvs[i2], positions[i0], positions[i1], positions[i2],
        normals[i0], normals[i1], normals[i2], transform, inverseTranspose,
            raytracerScene, pixelWidth, resolution,
            texelCallback, callbackParameters);
    }
}

// Sample AO directly at each vertex. This can be used as an AO approximation on its
// own but also as a baseline for raytraceOverTriangleSamples.
// DEPRECATED
// Note: per-vertex baking requires that every primitive is unique in the scene AND
// that no primitives share vertices with other primitives.
function raytraceAtVertices(primitive, meshPrimitiveID, parameters, node) {
    var raytracerScene = parameters.raytracerScene;
    var aoBuffer = raytracerScene.aoBufferByPrimitive[meshPrimitiveID];
    ...
    for (var i = 0; i < vertexCount; i++) {
        var position = Matrix4.multiplyByPoint(transform, positions[i], worldPositionScratch);
        var normal = Matrix4.multiplyByPointAsVector(inverseTranspose, normals[i], worldNormalScratch);
        normal = Cartesian3.normalize(normal, normal);

        // jitter the position out a little along the normal
        var jitter = Cartesian3.multiplyByScalar(normal, raytracerScene.nearCull, cartesian3Scratch);
        position = Cartesian3.add(jitter, position, position);

        // Raytrace
        aoBuffer.count[i] += numberSamples;
        aoBuffer.samples[i] += computeAmbientOcclusionAt(
            position, normal, numberSamples, sqrtNumberSamples,
            triangleSoup, raytracerScene.nearCull, raytracerScene.rayDepth);
    }
}
...
function raytraceAtTriangleCenters(primitive, meshPrimitiveID, parameters, node) {
    ...
    // For each center on a triangle, raytrace ambient occlusion.
    var hundredth = numTriangles / 100.0;
    var part = hundredth;

    console.log("doing centers....");
    for (var i = 0; i < numTriangles; i++) {
        if (i > part) {
            part += hundredth;
            console.log((i / numTriangles) * 100.0 + '%');
        }
...
function raytraceOverTriangleSamples(primitive, meshPrimitiveID, parameters, node) {
    ...
    console.log("doing additional samples....");
    for (var i = 0; i < numTriangles; i++) {
        if (i > part) {
            part += hundredth;
            console.log((i / numTriangles) * 100.0 + '%');
        ...
        pixelMarch(triangleUVs[0], triangleUVs[1], triangleUVs[2],
...
function planeSampleCallback(parameters, contribution, numbersamples, barycentric) {
    ...
    var b0 = barycentric.x + CesiumMath.EPSILON7; // avoid divide by zero without branching
    var b1 = barycentric.y + CesiumMath.EPSILON7;
    var b2 = barycentric.z + CesiumMath.EPSILON7;
    var baseline0 = aoBuffer.samples[i0] / aoBuffer.count[i0];
    var baseline1 = aoBuffer.samples[i1] / aoBuffer.count[i1];
    var baseline2 = aoBuffer.samples[i2] / aoBuffer.count[i2];

    var normalizedContribution = contribution / numbersamples;

    // Make a new AO estimate for each vertex based on the old estimate and this sample
    var estimate0 = CesiumMath.clamp((normalizedContribution - baseline1 * b1 - baseline2 * b2) / b0, 0.0, 1.0);
    var estimate1 = CesiumMath.clamp((normalizedContribution - baseline0 * b0 - baseline2 * b2) / b1, 0.0, 1.0);
    var estimate2 = CesiumMath.clamp((normalizedContribution - baseline1 * b1 - baseline0 * b0) / b2, 0.0, 1.0);

    // Update samples. Denormalize to prevent these samples from being overwhelemed by existing data.
    aoBuffer.samples[i0] += normalizedContribution * numbersamples;
    aoBuffer.samples[i1] += normalizedContribution * numbersamples;
    aoBuffer.samples[i2] += normalizedContribution * numbersamples;
...
function flattenTriangle(positions, results) {
    ...
    for (var i = 1; i < 3; i++) {
        // Compute x/y coordinates by getting distance to normalized y and x axes in 3D
        results[i].x = pointLineDistanceParametric(positions[i], positions[0], yAxis);
        results[i].y = pointLineDistanceParametric(positions[i], positions[0], xAxis);
    }
...
function pointLineDistanceParametric(point, position, direction) {
    var p1 = position;
    var p2 = Cartesian3.add(p1, direction, pointOnLineScratch);
    var p3 = point;
    var length2 = Cartesian3.magnitudeSquared(direction);
