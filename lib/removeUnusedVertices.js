            ...
            if (defined(primitives)) {
                ...
                for (var i = 0; i < primitiveLength; i++) {
                    ...
                    if (defined(primitive.indices)) {
                        ...
                        var indexStride = 2; // default to SHORT
                        ...
                        while (accessorIndex < accessorSourceLength) {
                            ...
                            switch(indexStride) {
                                case 2:
                                    ...
                                    break;
                                case 1:
                                    index = accessorSource[accessorIndex];
                                    break;
                            }
                        ...
                        for (var attributeId in attributes) {
                            if (attributes.hasOwnProperty(attributeId)) {
                                ...
                                if (Object.keys(attributeView.extras._pipeline.accessors).indexOf(attributes[attributeId]) === -1) {
                                    var attributeStride = 1; // Default to width of BYTE and UNSIGNED BYTE
                                    if (attributeAccessor.byteStride > 0) {
                                        attributeStride = attributeAccessor.byteStride;
                                    }
                                    else if (defined(attributeAccessor.type)){
                                        // If an attribute type is defined, a byteStride of 0 is equivalent to a byteStride
                                        // of BYTE_COUNT(type). The attributeStride is the width of the attribute type.
                                        var componentType = attributeAccessor.componentType;
                                        var componentWidth = 4; // default to FLOAT
                                        ...
                                        switch (attributeAccessor.type) {
                                            case 'SCALAR':
                                                attributeStride = componentWidth;
                                                break;
                                            case 'VEC2':
                                                attributeStride = componentWidth * 2;
                                                break;
                                            case 'VEC3':
                                                attributeStride = componentWidth * 3;
                                                break;
                                            case 'VEC4':
                                                attributeStride = componentWidth * 4;
                                                break;
                                            case 'MAT2':
                                                attributeStride = componentWidth * 4;
                                                break;
                                            case 'MAT3':
                                                attributeStride = componentWidth * 9;
                                                break;
                                            case 'MAT4':
                                                attributeStride = componentWidth * 16;
                                                break;
                                        }
                                    }
                                    else {
        ...
        if (bufferViews.hasOwnProperty(bufferViewId)) {
            ...
            if (defined(bufferView.extras._pipeline.indexIntervals)) {
                ...
                while (sourceIndex < viewEnd) {
                    ...
                    switch(indexStride) {
                        case 2:
                            index = indexSource.readUInt16LE(sourceIndex);
                            break;
                        case 1:
                            index = indexSource[sourceIndex];
                            break;
                    }
                        ...
                        if (index >= offsetArray[k][0]) {
                            switch(indexStride) {
                                case 2:
                                    ...
                                    break;
                                case 1:
                                    indexSource.writeUInt8(index - offsetArray[k][1], sourceIndex);
                                    break;
                            }
                            break;
                        }
