...
var computeStatistics = require('./computeStatistics');
...
Pipeline.processJSONWithExtras  = function(gltfWithExtras, options) {
    ...
    // Generate stats for unoptimized input
    if (options.stats) {
        computeStatistics.printStatistics(gltfWithExtras, options.inputPath);
    }
    ...
    return Promise.all(waitForStages)
        ...
        .then(function() {
            ...
            // Generate stats for optimized input
            if (options.stats) {
                computeStatistics.printStatistics(gltfWithExtras, options.outputPath);
            }
