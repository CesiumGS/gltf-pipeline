...
function writeGltf(gltf, outputPath, isEmbedded, isImageEmbedded, createDirectory, callback) {
    ...
    async.each(['buffers', 'images', 'shaders'], function(name, asyncCallback) {
            writeSource(gltf, basePath, name, isEmbedded, isImageEmbedded, asyncCallback);
        }, function(err) {
            if (err) {
                if (callback) {
                    callback(err);
