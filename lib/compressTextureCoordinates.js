...
function compressTextureCoordinates(gltf) {
    ...
    var texCoordAccessors = getTextureCoordinateAccessors(gltf);
    var patchedTechniques = {};
    var patchedPrograms = {};
    for (var texCoordAccessorId in texCoordAccessors) {
        if (texCoordAccessors.hasOwnProperty(texCoordAccessorId)) {
            var accessor = accessors[texCoordAccessorId];
            var primitive = texCoordAccessors[texCoordAccessorId];
            if (canCompress(accessor, primitive)) {
                ...
                for (var j = 0; j < numElements; j++) {
                    ...
                    writeBufferComponentType(source, newComponentType, compressed, byteOffset);
...
function getTextureCoordinateAccessors(gltf) {
...
function patchProgram(gltf, programId, texCoordVariableName) {
